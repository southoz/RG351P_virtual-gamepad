	...
	public:
		virtual bool translate_event(input_event& ev) {
			...
			case EV_KEY:{
				...
				// dpad
				case BTN_DPAD_UP: { ev.type = EV_ABS; ev.code = ABS_HAT0Y; ev.value = (ev.value) ? -1 : 0;} break; // Up
				case BTN_DPAD_DOWN: { ev.type = EV_ABS; ev.code = ABS_HAT0Y; ev.value = (ev.value) ? 1 : 0;} break; // Down
				case BTN_DPAD_LEFT: { ev.type = EV_ABS; ev.code = ABS_HAT0X; ev.value = (ev.value) ? -1 : 0;} break; // Left
				case BTN_DPAD_RIGHT: { ev.type = EV_ABS; ev.code = ABS_HAT0X; ev.value = (ev.value) ? 1 : 0;} break; // Right
				// do not report any other button
			...
			case EV_ABS: {
				switch(ev.code) {
				// left stick
				case ABS_X: { ev.code = ABS_X; ev.value = (ev.value*(32768*2)/4095-32768) * -1;} break;
				case ABS_Y: { ev.code = ABS_Y; ev.value = (ev.value*(32768*2)/4095-32768) * -1;} break;
				// right stick
				case ABS_RX: { ev.code = ABS_RX; ev.value = ev.value*(32768*2)/4095-32768;} break;
				case ABS_RY: { ev.code = ABS_RY; ev.value = ev.value*(32768*2)/4095-32768;} break;
				// do not report any other axis
				default:
					return false;
				}
